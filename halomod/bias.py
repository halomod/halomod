...
class Bias(Component):
    ...
    def __init__(self, nu, delta_c=1.686, m=None, mstar=None, delta_halo=None, n=None,
                 Om0=None, sigma_8=None, h=None, **model_parameters):
        """
...
class Jing98(Bias):
    ...
    def bias(self):
        ...
        b = self.params['b']
        c = self.params['c']
        return (a / nu ** 4 + 1) ** (b - c * self.n) * (1 + (nu ** 2 - 1) / self.delta_c)
...
class SMT01(Bias):
    ...
    def bias(self):
        ...
        b = self.params['b']
        c = self.params['c']
        return 1 + (sa * (a * nu) + sa * b * (a * nu) ** (1 - c) - (a * nu) ** c / (
                    (a * nu) ** c + b * (1 - c) * (1 - c / 2))) / (self.delta_c * sa)
...
class Seljak04(Bias):
    ...
    """
    _defaults = {"a": 0.53, "b": 0.39, "c": 0.45, "d": 0.13, "e": 40, "f": 5e-4, "g": 1.5}
...
class Seljak04_Cosmo(Seljak04):
    ...
    def bias(self):
        ...
        a1 = self.params['a1']
        a2 = self.params['a2']
        return b + np.log10(self.m / self.mstar) * (a1 * (self.Om0 - 0.3 + self.n - 1) +
...
class Tinker10_PBsplit(Bias):
    ...
    def bias(self):
        ...
        return 1 + (gamma * self.nu - (1 + 2 * eta)) / self.delta_c + 2 * phi / self.delta_c / (
                    1 + (beta ** 2 * self.nu) ** phi)
