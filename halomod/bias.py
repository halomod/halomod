...
class Bias(Component):
    ...
    """
    _defaults = {}
    def __init__(self, nu, delta_c, m = None,mstar=None,delta_halo=None, n=None,
...
class SMT01(Bias):
    ...
    def bias(self):
        ...
        b = self.params['b']
        c = self.params['c']
        return 1+(sa*(a*nu) + sa*b*(a*nu)**(1-c) - (a*nu)**c/((a*nu)**c + b*(1-c)*(1-c/2)))/(self.delta_c*sa)
...
class Tinker10_PBsplit(Bias):
    ...
    def bias(self):
        ...
        return 1 + (gamma*self.nu - (1+2*eta))/self.delta_c + 2*phi/self.delta_c/(1+(beta**2 * self.nu)**phi)
