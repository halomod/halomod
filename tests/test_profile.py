...
def test_virial_velocity(thm, thmnum):
    ...
    r = np.array([0.1, 1.0])
    attnum = thmnum.halo_profile.virial_velocity
    att = thm.halo_profile.virial_velocity
...
def test_h(thm, thmnum):
    m = np.array([1e12, 1e13])
    attnum = thmnum.halo_profile._h
    att = thm.halo_profile._h
    cm = thm.halo_profile.cm_relation
...
def test_u(thm, thmnum):
    k = np.array([0.1, 1])
    m = np.array([1e12, 1e13])
    attnum = thmnum.halo_profile.u
    att = thm.halo_profile.u
...
def test_lam(thmnum, thm):
    r = np.array([0.1, 1])
    m = np.array([1e12, 1e13])
    attnum = thmnum.halo_profile.lam
    att = thm.halo_profile.lam
...
def test_get_r_variable(thmnum):
    r = np.array([0.1, 1])
    m = np.array([1e12, 1e13])
    attnum = thmnum.halo_profile._get_r_variables
    with pytest.raises(ValueError):
        attnum(r, m, coord="m")
