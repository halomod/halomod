...
!            if(thisnk.gt.largest_nk)then
!                largest_nk = thisnk
 !           end if
        !write(*,*) "thisnk max:", thisnk
!        end do
!        allocate(thislnk(largest_nk),thispower(largest_nk),integ(largest_nk))
!        corr = 0.d0
!        do i=1,nr
!            rr = r(i)
...
!            corr(i) = (0.5/pi**2) * corr(i)
 !       end do
        !deallocate(thislnk,thispower,integ)
  ...
  !  end subroutine power_to_corr_matrix
    ...
    subroutine noexclusion(nm,nk,nr,m,bias,ntot,dndm,lnk,dmpower,u,r,nbar,corr)
        ! No halo exclusion, not scale dependent.
        integer, intent(in) :: nm,nk,nr
        real(8), intent(in) :: m(nm),bias(nm), ntot(nm), dndm(nm)
        real(8), intent(in) :: u(nm,nk),dmpower(nk),lnk(nk)
        real(8), intent(in) :: r(nr)
        real(8), intent(in) :: nbar
        real(8), intent(out):: corr(nr)

        real(8) :: integrand_m(nm), pg2h(nk), integrand(nm)
        integer :: i

        integrand_m = m*dndm * ntot * bias
        !write(*,*) "integrand_m_lo: ", integrand_m(1:25)/m(1:25)
        !write(*,*) "integrand_m_hi: ", integrand_m(nm-25:nm)/m(nm-25:nm)

        do i=1,nk
            integrand = integrand_m * u(:,i)
         !   if (i==1.or.i==1000) write(*,*) "intm after u lo", integrand_m(1:25)/m(1:25)
         !   if (i==1.or.i==1000) write(*,*) "intm after u hi", integrand_m(nm-25:nm)/m(nm-25:nm)
         !   if (i==1.or.i==1000) write(*,*) "u lo", u(1:25,i)
         !   if (i==1.or.i==1000) write(*,*) "u hi", u(nm-25:nm,i)
            call simps(log(m(2))-log(m(1)),integrand,pg2h(i))
        end do

        pg2h = dmpower * pg2h**2 / nbar**2

        call power_to_corr(nr,nk,r,pg2h,lnk,myN,myh,corr)

    end subroutine noexclusion

    subroutine noexclusion_sd(nm,nk,nr,m,bias,ntot,dndm,lnk,dmpower,u,r,dmcorr,nbar,corr)

        ! No halo exclusion, but scale dependent.
        integer, intent(in) :: nm,nk,nr
        real(8), intent(in) :: m(nm),bias(nm), ntot(nm), dndm(nm)
        real(8), intent(in) :: u(nm,nk),lnk(nk),dmpower(nk)
        real(8), intent(in) :: r(nr),dmcorr(nr)
        real(8), intent(in) :: nbar
        real(8), intent(out):: corr(nr)

        real(8) :: integrand_m(nm), pg2h(nk,nr),ng_integrand(nm), integrand(nm)

        integer :: i,j
        !write(*,*) "in fortran"
        !only difference is we have a scale-dependent bias
        ...
        do j=1,nr
            ...
            do i=1,nk
                ...
                call simps(log(m(2))-log(m(1)),integrand,pg2h(i,j))
    ...
    end subroutine noexclusion_sd

    subroutine schneider(nm,nk,nr,m,bias,ntot,dndm,lnk,dmpower,u,r,nbar,corr)

        ! Schneider exclusion, not scale dependent.
        integer, intent(in) :: nm,nk,nr
        real(8), intent(in) :: m(nm),bias(nm), ntot(nm), dndm(nm)
        real(8), intent(in) :: u(nm,nk),dmpower(nk),lnk(nk)
        real(8), intent(in) :: r(nr)
        ...
        real(8) :: integrand_m(nm), pg2h(nk),x(nk), integrand(nm)
        integer :: i
        ...
        x = exp(lnk)*3.d0
        pg2h = (3 * (sin(x) - x * cos(x)) / x ** 3)*dmpower * pg2h**2 / nbar**2
        call power_to_corr(nr,nk,r,pg2h,lnk,myN,myh,corr)

    end subroutine schneider

    subroutine schneider_sd(nm,nk,nr,m,bias,ntot,dndm,lnk,dmpower,u,r,dmcorr,nbar,corr)

        ! Schneider exclusion, scale dependent.
        integer, intent(in) :: nm,nk,nr
        real(8), intent(in) :: m(nm),bias(nm), ntot(nm), dndm(nm)
        real(8), intent(in) :: u(nm,nk),lnk(nk),dmpower(nk)
        real(8), intent(in) :: r(nr),dmcorr(nr)
        ...
        real(8) :: integrand_m(nm), pg2h(nk,nr),x(nk),window(nk),ng_integrand(nm)
        real(8) :: integrand(nm)
        integer :: i,j
        ng_integrand = m*dndm * ntot
        x = exp(lnk)*3.d0
        ...
        do j=1,nr
            ...
            pg2h(:,j) = window*dmpower * pg2h(:,j)**2 / nbar**2
        end do
        call power_to_corr_matrix(nr,nk,r,pg2h,lnk,myN,myh,corr)

    end subroutine schneider_sd

    subroutine sphere(nm,nk,nr,m,bias,ntot,dndm,lnk,dmpower,u,r,nbar,dhalo,rhob,corr)
        ...
        real(8) :: integrand_m(nm), pg2h(nk,nr), integrand(nm),mlim,ngdash(nr)
        integer :: i,j
        ...
        do j=1,nr
            ...
            do i=1,nk
                where (m<mlim) integrand = integrand_m * u(:,i)
                call simps(log(m(2))-log(m(1)),integrand,pg2h(i,j))
            end do
    ...
    subroutine sphere_sd(nm,nk,nr,m,bias,ntot,dndm,lnk,dmpower,u,r,dmcorr,nbar,dhalo,rhob,corr)
        ...
        real(8) :: ng_integrand(nm),integrand_m(nm), pg2h(nk,nr), integrand(nm)
        real(8) :: mlim,ngdash(nr)
        integer :: i,j
        ...
        do i=1,nr
            corr(i) = (ngdash(i)/nbar)**2*(1.d0+corr(i))-1.d0
            if(corr(i)<-0.d0.and.r(i)<10.d0) corr(i) = 1e-15
        end do

    end subroutine sphere_sd

    subroutine ng_matched(nm,nk,nr,m,bias,ntot,dndm,lnk,dmpower,u,r,nbar,dhalo,rhob,corr)
        ...
        real(8) :: integrand_m(nm), pg2h(nk,nr), integrand(nm),val,dlogm,ng
        real(8) :: rvir(nm), ng_integrand(nm)
        real(8), allocatable :: prob(:,:), outersum(:,:),integprod(:,:)
        integer :: i,j, final_index

        integrand = 0.d0
        ng_integrand = m*dndm * ntot
        integrand_m = ng_integrand * bias
        call virial_radius(m,rhob,dhalo,rvir)
        dlogm = (log(m(2))-log(m(1)))
        allocate(outersum(nm,nm),integprod(nm,nm))
        do i=1,nm
        ...
        do j=1,nr
            ...
            prob = prob * integprod
            call dblsimps(prob,nm,nm,dlogm,dlogm,ng)
            ng = sqrt(ng)
            ...
            do i=1,nk
                integrand = integrand_m * u(:,i)
                call simps(log(m(2))-log(m(1)),integrand(1:final_index),pg2h(i,j))
            end do
    ...
    end subroutine ng_matched

    subroutine ng_matched_sd(nm,nk,nr,m,bias,ntot,dndm,lnk,dmpower,u,r,dmcorr,nbar,dhalo,rhob,corr)
    ...
    end subroutine ng_matched_sd

    subroutine ellipsoid(nm,nk,nr,m,bias,ntot,dndm,lnk,dmpower,u,r,nbar,dhalo,rhob,corr)
        ...
        real(8) :: integrand_m(nm), pg2h(nk,nr), integrand(nm),dlogm
        real(8) :: rvir(nm),ng_integrand(nm)
        real(8), allocatable :: prob(:,:), outersum(:,:), integprod(:,:)
        integer :: i,j, k
        ...
        do j=1,nr
            ...
            do i=1,nk
                integrand = integrand_m * u(:,i)
                do k=1,nm
                    integprod(:,k) =  integrand(k)*integrand
                end do
                prob = prob * integprod
    ...
    subroutine ellipsoid_sd(nm,nk,nr,m,bias,ntot,dndm,lnk,dmpower,u,r,dmcorr,nbar,dhalo,rhob,corr)
    ...
    subroutine twohalo(nm,nk,nr,m,bias,ntot,dndm,lnk,dmpower,u,r,dmcorr,nbar,&
        ...
        dlogm = (log(m(2))-log(m(1)))
        ng_integrand = m*dndm * ntot
        if(.not.scale_dep) integrand_m = ng_integrand * bias
